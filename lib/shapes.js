...
class Triangle extends Shape {
    render(logoText, textColor) {
        const shapeColor = this.color;
        const textX = 150;
        const textY = 121;
        return `<polygon points="150,18 244,182 56, 182" fill="${shapeColor}" />
          <text x="${textX}" y="${textY}" font-size="60" fill="${textColor}" text-anchor="middle" dominant-baseline="central">
            ${logoText}
          </text>`;
    }
}
...
function generateLogo(answers) {
    ...
    svgString += `
      <text x="150" y="100" font-size="60" fill="${textColor}" text-anchor="middle" dominant-baseline="middle">
        ${logoText}
      </text>`;
